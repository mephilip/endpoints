...
class Config {
	...
	function create_post_type() {
	    ...
	    $labels = array(
	        'name'               => _x( 'Dog Food', 'post type general name', '' ),
	        'singular_name'      => _x( 'Product', 'post type singular name', '' ),
	        'menu_name'          => _x( 'Dog Food', 'admin menu', '' ),
	    ...
	    register_post_type( 'dog_food', $args );
	}
    ...
    public static function enqueue_assets() {
		...
		// Font awesome stylesheet for icon use ~ Thomas
		// TODO ~ Source 
		wp_enqueue_style( 'app-css', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css' );
		
        // WordPress's required styles.css (will override compiled SASS)
        // Not Necessary to enqueue ~ Thomas
        ...
        wp_enqueue_script( 'd3',  NV_JS.'/d3.min.js', array(), '1.0.2' );

		// TopoJSON 
        wp_enqueue_script( 'topojson',  NV_JS.'/topojson.min.js', array(), '1.0.2' );
        
        // 
        wp_enqueue_script( 'queue',  NV_JS.'/queue.v1.min.js', array(), '1.0.2' );
        ...
        // Theme Features ~ Thomas 
        wp_enqueue_script( 'theme-features',  NV_JS.'/features.js', array(), false, true );
        
        wp_enqueue_script('mediaAlpha', 'http://insurance.mediaalpha.com/js/serve.js', array(), false, true);
        wp_enqueue_script('highcharts', 'http://code.highcharts.com/highcharts.js', array(), false, true);

        // JS for State pages - Dennis 
        wp_enqueue_script( 'state-features',  NV_JS.'/state.js', array(), false, true );

        // Useful Localization of Javascript Variables ~ Thomas
        wp_localize_script( 'theme-app', 'Variables', array(
			// URL to wp-admin/admin-ajax.php to process data
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'home_url' => home_url(),
			'current_url' => "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]",
			'theme_url' => get_template_directory_uri(),
			'loading_icon' => __('<i class="fa fa-spinner fa-pulse"></i>'),
			'loading_image' => "<img class='loading_image' src='" . NV_IMG . "/loading_image.gif'/>",
